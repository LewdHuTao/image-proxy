...
app.post("/upload", upload.single("image"), async (req, res) => {
  try {
    const imagePath = Buffer.from(req.body.image, "base64");
    const uniqueFilename = uuidv4() + ".jpg";
    const processedImageUrl = await processImage(imagePath, uniqueFilename);
...
async function clearUploadsFolder() {
  try {
    ...
    const now = Date.now();
    const fiveHours = 8 * 60 * 1000;const express = require("express");
    const multer = require("multer");
    const bodyParser = require("body-parser");
    const sharp = require("sharp");
    const { v4: uuidv4 } = require("uuid");
